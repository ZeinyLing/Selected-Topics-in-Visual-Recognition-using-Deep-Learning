...
with torch.no_grad():
    ...
    for imgs, names in loader:
        ...
        for model in models:
            ...
            all_preds.append(preds.unsqueeze(0))  # (1, batch)

        # shape: (num_models, batch)
        all_preds = torch.cat(all_preds, dim=0)

        # majority vote
        final_preds = []

        for i in range(all_preds.shape[1]):  # loop over batch
            votes = all_preds[:, i]  # predictions from all models
            ...
            final_preds.append(winner.item())

        for name, p in zip(names, final_preds):
