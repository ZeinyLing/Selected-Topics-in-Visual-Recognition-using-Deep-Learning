...
# 如果你的 train 檔名不是 train.py，例如 train_v22.py，要改這行
from train_V3 import PromptIRStrong
...
def pil_to_tensor(img):
    """
    PIL RGB image -> torch tensor [3, H, W], float32, range 0~1
    """
...
def tensor_to_uint8_chw(x):
    """
    Input:
        x: torch.Tensor [3, H, W], range 0~1

    Output:
        np.ndarray [3, H, W], dtype uint8
    """
...
def main():
    ...
    print("=" * 60)
    print("Inference Strong PromptIR V2.2 - Detail + Frequency")
    print("=" * 60)
    print("Device:", DEVICE)
    print("Loaded model:", BEST_MODEL_PATH)
    print("Best validation PSNR:", ckpt.get("best_psnr", "N/A"))
    print("Model name:", ckpt.get("model_name", "N/A"))
    print("=" * 60)
    ...
    for img_tensor, filename in tqdm(test_dataset, desc="Inference"):
        ...
        # Save to pred.npz format: key = filename, value = [3, H, W], uint8
        pred_dict[filename] = restored_chw

        # Save restored PNG for visual checking
